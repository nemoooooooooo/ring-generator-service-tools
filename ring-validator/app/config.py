...
def _default_blender_executable() -> Path:
    candidates: list[str] = [
        os.getenv("RING_VAL_BLENDER_EXECUTABLE", "").strip(),
...
class ValidatorSettings(BaseSettings):
    model_config = SettingsConfigDict(
        env_prefix="RING_VAL_",
        env_file=".env",
        env_file_encoding="utf-8",
        extra="ignore",
    )

    service_name: str = "ring-validator-service"
    host: str = "0.0.0.0"
    port: int = 8104
    log_level: str = "INFO"

    # LLM API keys (needed for validation LLM calls)
    anthropic_api_key: str = Field(default_factory=lambda: os.getenv("ANTHROPIC_API_KEY", ""))
    gemini_api_key: str = Field(default_factory=lambda: os.getenv("GEMINI_API_KEY", ""))
    gemini_model: str = Field(default_factory=lambda: os.getenv("GEMINI_MODEL", "gemini-3-pro-preview"))

    # Blender (needed for re-rendering corrected code)
    ...
    # Master prompt (loaded at startup, used in validation prompt)
    ...
    sync_wait_timeout_seconds: int = Field(default=300, ge=30, le=600)
