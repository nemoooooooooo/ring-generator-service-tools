...
def _default_blender_executable() -> Path:
    candidates: list[str] = [
        os.getenv("RING_GEN_BLENDER_EXECUTABLE", "").strip(),
...
class RingGenSettings(BaseSettings):
    model_config = SettingsConfigDict(
        env_prefix="RING_GEN_",
        env_file=".env",
        env_file_encoding="utf-8",
        extra="ignore",
    )

    service_name: str = "ring-generator-service"
    host: str = "0.0.0.0"
    port: int = 8102
    log_level: str = "INFO"
